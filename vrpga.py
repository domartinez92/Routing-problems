...
def decodeVRP(chromosome, trucks,num_trucks, cities):    
    list=[]
...
def penalty_capacity(chromosome,trucks, capacity_trucks):
        actual = chromosome
        value_penalty = 0
        capacity_list = []
        index_cap = 0
...
def fitnessVRP(chromosome, trucks, capacity_trucks, distances):
    ...
    penalty_cap = penalty_capacity(actualChromosome, trucks, capacity_trucks)
...
def VRP(VRP_PROBLEM,k,capacity_trucks):   
    def first_part_GA(k, capacity_trucks):
        cont  = 0
    ...
    first_part_GA(k, capacity_trucks)    